## **Instruções para 8/05:**

### <span style="color:yellow"> Introdução à segmentação de imagem </span>

#### Leitura do resumo do Medium:

https://medium.com/@raj.pulapakura/image-segmentation-a-beginners-guide-0ede91052db7

#### Leitura do resumo da IBM:

https://www.ibm.com/think/topics/image-segmentation

#### Leitura do pdf (páginas 1 a 7), focando em k-means clustering segmentation:

[Click here to view the PDF](image_segmentation.pdf)

#### Investigar notebooks de k-means clustering segmentation:

[Click here to view the notebook](k_means_clustering.ipynb)

### <span style="color:yellow"> Convolução e Redes Convolucionais: </span>

#### Leitura do resumo do Medium:

https://gakinolaalli.medium.com/convolution-in-image-processing-6d382da5048e

#### Leitura do resumo do 8thlight:

https://8thlight.com/insights/what-is-a-convolution-how-to-teach-machines-to-see-images

#### Ver video sobre CNNs:

https://www.youtube.com/watch?v=YGILT182T6w

## **Instruções para 15/05:**

### <span style="color:yellow"> U-net architecture: </span>

#### Leitura do resumo do Medium:

https://medium.com/@alejandro.itoaramendia/decoding-the-u-net-a-complete-guide-810b1c6d56d8

#### Leitura do artigo que introduziu a arquitetura U-net:

Leitura **leve** e **dinâmica** do artigo original. Não fiquem assustados. Vai ser uma experiência diferente, mas divertida. :)

[Click here to view the PDF](ronneberger_2015.pdf)

#### Investigar notebooks de Convolução e de U-net:

[Click here to view the notebook](U-net_architecture.ipynb)

### <span style="color:yellow"> Biomedisa DL: </span>

#### Leitura do artigo sobre Deep Learning (do Biomedisa):

Ler somente as seções "Introduction" e "Material and Methods".
[Click here to view the PDF](biomedisa_dl.pdf)

<span style="color:orange">
Observação) 

Ao fim dos estudos, preparar uma apresentação para o dia 22/05 resumindo o conteúdo aprendido: 

- Introdução à segmentação de imagem
- Convolução e Redes Convolucionais
- U-net Architecture
- Biomedisa DL

Vocês podem dividir entre vocês os temas caso queiram ou fazer em conjunto.
</span>